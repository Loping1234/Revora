...
export function SalesDataPanel({ handleUpload, handleDownloadPanelReport, selectedFile, setSelectedFile, uploadState, uploadMessage, uploadSummary, totalSalesRecords }) {
  ...
  return (
    <section className="h-full overflow-auto rounded-lg border border-slate-200 bg-white p-4">
      ...
      <form className="mt-4 shrink-0 flex flex-col gap-3 sm:flex-row sm:items-center" onSubmit={handleUpload}>
        <input
          accept=".csv,text/csv"
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 file:mr-3 file:rounded-md file:border-0 file:bg-slate-950 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-white sm:max-w-md"
          onChange={(event) => setSelectedFile(event.target.files?.[0] || null)}
          type="file"
        />
        <button
          className="inline-flex h-10 w-full min-w-28 items-center justify-center whitespace-nowrap rounded-md bg-slate-950 px-4 text-sm font-medium text-white disabled:cursor-not-allowed disabled:bg-slate-400 sm:w-auto"
          disabled={uploadState === "uploading"}
          type="submit"
        >
          {uploadState === "uploading" ? "Uploading" : "Upload CSV"}
        </button>
      </form>
        ...
        <section className="mt-4 min-h-0 overflow-auto rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
            <div className="rounded-md border border-slate-200 bg-white p-3">
              ...
              <p className="mt-1 text-lg font-semibold">{uploadSummary.importedRows}</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-white p-3">
            ...
            <div className="rounded-md border border-slate-200 bg-white p-3">
              <p className="text-xs uppercase text-slate-500">New products</p>
              <p className="mt-1 text-lg font-semibold">{uploadSummary.newProductsCreated ?? 0}</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-white p-3">
              <p className="text-xs uppercase text-slate-500">Matched</p>
              <p className="mt-1 text-lg font-semibold">{uploadSummary.existingProductsMatched ?? 0}</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-white p-3">
              <p className="text-xs uppercase text-slate-500">Duplicates</p>
              <p className="mt-1 text-lg font-semibold">{uploadSummary.duplicateRowsSkipped ?? 0}</p>
            </div>
            <div className="rounded-md border border-slate-200 bg-white p-3">
